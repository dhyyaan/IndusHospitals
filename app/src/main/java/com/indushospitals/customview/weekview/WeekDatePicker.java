package com.indushospitals.customview.weekview;
...
public class WeekDatePicker extends View {
    ...
    public WeekDatePicker(Context context, AttributeSet attrs, int defStyle) {
        ...
        try {
            ...
            dayNameLength = a.getInt(R.styleable.WeekDatePicker_wdp_dayNameLength,3);

            boolean firstDayOfWeekUsingCurrrentdate = a.getBoolean(R.styleable.WeekDatePicker_wdp_firstDayOfWeekUsingCurrrentdate,false);
            int dayOfWeek =0;
            if(firstDayOfWeekUsingCurrrentdate){dayOfWeek =  LocalDate.now().getDayOfWeek().getValue();
            }else{dayOfWeek = a.getInt(R.styleable.WeekDatePicker_wdp_firstDayOfWeek, DayOfWeek.SUNDAY.getValue());}
